  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button className="inline-flex items-center gap-1.5 px-4 py-2.5 text-[13px] font-semibold text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-all">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 p-5">
          <h3 className="text-[13px] font-semibold text-slate-900 dark:text-slate-100 mb-3">Budget vs Spend</h3>
          <MiniChart data={[10, 25, 40, 55, 70, 85, 100].map((v) => totalBudget * v / 100)} color="#4f46e5" height={100} />
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 p-5">
          <h3 className="text-[13px] font-semibold text-slate-900 dark:text-slate-100 mb-3">Phase Completion Trend</h3>
          <MiniChart data={[0, 15, 25, 35, 45, 60, avgComp]} color="#f59e0b" height={100} />
          <div className="flex items-center gap-4 mt-2 text-[11px] text-slate-400 dark:text-slate-500">
            <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-amber-500" /> Completion %</span>
          </div>
        </div>
      </div>
              ...
              return (
                <div key={phase.id} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                  ...
                  <div className="flex items-center gap-2 sm:gap-0">
                    <div className="text-[13px] text-slate-500 dark:text-slate-400 sm:w-24 sm:text-right">{formatNaira(phase.budgetSpent)}</div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 p-5">
          ...
          <div className="space-y-2">
            {[
              { label: "Budget Variance", value: "Within range", good: true },
              { label: "Schedule Variance", value: "On track", good: true },
              { label: "Dispute Status", value: "1 open dispute", good: false },
              { label: "Evidence Coverage", value: "85% verified", good: true },
            ].map((r, i) => (
